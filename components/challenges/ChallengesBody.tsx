...
// The actual content of the Challenges feature — one always-open "Raffle",
// one info-only "Spotlight" (Creator of the Week), and one rotating
// "Featured" challenge creators tap to unlock. Rendered inside the
// collapsible row embedded in Creator Hub (see CreatorHubChallenges.tsx) —
// there is no standalone Challenges page/tab.
//
// The raffle does NOT have its own "link your recreation" box — a creator
// already does that from the Weekly Opportunity spotlight on the Resource
// Hub, so this just shows read-only status and a link to there instead of
// a second, confusing place to paste the same link.
...
export default function ChallengesBody({
  ...
  // "coming-soon" only makes sense as a teaser for the featured challenge
  // (see lib/data/types.ts) — for raffle/spotlight it behaves like
  // "retired" and stays hidden from creators.
  const pool = adminMode
    ? challenges
    : challenges.filter((c) => c.status === "active" || (c.status === "coming-soon" && c.kind === "featured"));
  ...
  const featuredDaysStart = featured ? daysLeft(featured.cycleStart) : null;

  if (editingChallenge) {
    return <ChallengeForm initial={editingChallenge} onSave={(data) => updateChallenge(editingChallenge.id, data)} onCancel={() => setEditingId(null)} />;
  }
  if (adding) {
    return <ChallengeForm onSave={addChallenge} onCancel={() => setAdding(false)} />;
  }
      ...
      {featured && (
        <div>
          ...
          {featured.status === "coming-soon" ? (
            <div className="relative overflow-hidden rounded-[24px] border border-dashed border-border bg-bg p-6 text-center">
              <p className="text-2xl">🎯</p>
              <p className="mt-2 text-base font-bold text-text">
                {featured.comingSoonMessage ||
                  `New challenge ${featuredDaysStart !== null ? (featuredDaysStart <= 0 ? "starting soon" : `in ${featuredDaysStart}d`) : "coming soon"}`}
              </p>
              {!featured.comingSoonMessage && (
                <p className="mt-1 text-xs text-text-muted">Details drop when it goes live — check back then.</p>
              )}
              {adminMode && (
                <EditControls
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          {raffle && (
            <div className="card p-4">
              ...
              {(raffle.prizeText || raffle.prizeImageUrl) && (
                <div className="mt-2.5 flex items-center gap-2.5 rounded-xl bg-accent-tint p-2">
                  {raffle.prizeImageUrl ? (
                    // eslint-disable-next-line @next/next/no-img-element -- arbitrary uploaded prize photo
                    <img src={raffle.prizeImageUrl} alt="" className="h-10 w-10 shrink-0 rounded-lg object-cover" />
                  ) : (
                    <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-bg text-lg" style={{ color: "var(--accent)" }}>🎁</span>
                  )}
                  <div className="min-w-0">
                    <p className="text-[10px] font-bold uppercase tracking-wide" style={{ color: "var(--accent)" }}>Prize</p>
                    <p className="truncate text-sm font-bold text-text">{raffle.prizeText || "TBA"}</p>
                  </div>
                </div>
              )}
              ...
              {creator && (
                <Link
                  href="/"
                  className="mt-2.5 flex items-center justify-center gap-1.5 rounded-full bg-text py-2 text-xs font-bold text-bg transition-opacity hover:opacity-90"
                >
                  Go to this week&apos;s opportunity to link it →
                </Link>
              )}
          ...
          {spotlight && (
            <div className="card p-4">
              ...
              {(spotlight.prizeText || spotlight.prizeImageUrl) && (
                <div className="mt-2.5 flex items-center gap-2.5 rounded-xl bg-accent-tint p-2">
                  {spotlight.prizeImageUrl ? (
                    // eslint-disable-next-line @next/next/no-img-element -- arbitrary uploaded prize photo
                    <img src={spotlight.prizeImageUrl} alt="" className="h-10 w-10 shrink-0 rounded-lg object-cover" />
                  ) : (
                    <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-bg text-lg" style={{ color: "var(--accent)" }}>💵</span>
                  )}
                  <div className="min-w-0">
                    <p className="text-[10px] font-bold uppercase tracking-wide" style={{ color: "var(--accent)" }}>Prize</p>
                    <p className="truncate text-sm font-bold text-text">{spotlight.prizeText || "TBA"}</p>
                  </div>
                </div>
              )}
